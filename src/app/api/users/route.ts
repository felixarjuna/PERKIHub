...
export async function POST(req: Request) {
  try {
    ...
    const { success, error } = await createUser(validatedData);
...
export async function PUT(req: Request) {
  try {
    ...
    const { success, error } = await updateUser(validatedParams.id, validatedData);
...
export async function DELETE(req: Request) {
  try {
    ...
    const validatedParams = userIdSchema.parse({ id });
    const { success, error } = await deleteUser(validatedParams.id);
    if (error) return NextResponse.json({ error }, { status: 500 });
